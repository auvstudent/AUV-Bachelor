    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if  event.button == 0: #press A button to smile
                test_input.done = 0
                test_input.direction = test_input.test1[0][0]
            elif event.button == 3:
                test_input.done = 1
                test_input.pointer = 0
                test_input.loc = 0
    ...
    while menue.location == "controller":#test code for controller input
        ...
        if gamepad != 0:
            ...
            while menue.location == "controller":
                ...
                if counter >= fps:
                    meny.get_lastline()
                    pid.uppdate(meny.yd,meny.zd)
                    
                    if test_input.done == 0:
                        print(test_input.direction)
                    else:
                        pass
                    test_input.move()
                    gamepad.left_joystick_float(x_value_float=-joy[0][0][0], y_value_float=joy[0][1][0])  # values between -1.0 and 1.0
                    gamepad.right_joystick_float(x_value_float=pid.output_x, y_value_float=pid.output_y)  # values between -1.0 and 1.0
                    gamepad.update()
                    #print(pid.output_x) # x = roll
                    #print(meny.zd)
                    counter = 0
                else:
                    counter += 1
                #meny.x meny.y meny.z
            ...
            if data.simulate_flag == 1:
                ...
                while menue.location == "test":
                    ...
                    if thing.timer < thing.end:
                        thing.timer +=5
                        if thing.timer >= 0:
                            thing.rotate((np.pi/180)*thing.roll_sim[thing.timer],(np.pi/180)*thing.pitch_sim[thing.timer],(np.pi/180)*thing.yaw_sim[thing.timer],0,0,0)
                    # rotate += (np.pi/180)
                    # if rotate >= 2*np.pi:
                    #     rotate = 0
                    # thing.rotate(0,0,rotate,0,0,0)
                    screen.blit(thing.board,(10,10))

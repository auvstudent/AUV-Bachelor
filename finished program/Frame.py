...
class wire_frame:
    def __init__(self,df):
        ...
        self.polygons = [[12,13,14,1,(0,0,255)],[12,14,15,1,(0,0,255)],[12,15,16,1,(0,0,255)],[12,16,17,1,(0,0,255)],[12,17,18,1,(0,0,255)],[12,18,19,1,(0,0,255)],
                         [5,13,14,-1,(255,0,0)],[5,6,14,1,(255,0,0)],
                         [9,17,18,-1,(0,255,0)],[9,10,18,1,(0,255,0)]]
        self.liste = []
        for i in range(len(self.polygons)):
            self.liste.append(False)
    ...
    def uppdate(self):
        ...
        for i in self.edges:
            ...
            pygame.draw.line(self.board, color,
                             (int(self.mid[0]+self.scale*self.pointsd[i[0]][0]),int(self.mid[1]+self.scale*self.pointsd[i[0]][1])),
                             (int(self.mid[0]+self.scale*self.pointsd[i[1]][0]),int(self.mid[1]+self.scale*self.pointsd[i[1]][1])),width=2)
        for i in range(len(self.polygons)):
            if self.liste[i] == True:
                pygame.draw.polygon(self.board,self.polygons[i][4],[(int(self.mid[0]+self.scale*self.pointsd[self.polygons[i][0]][0]),int(self.mid[1]+self.scale*self.pointsd[self.polygons[i][0]][1])),
                                                          (int(self.mid[0]+self.scale*self.pointsd[self.polygons[i][1]][0]),int(self.mid[1]+self.scale*self.pointsd[self.polygons[i][1]][1])),
                                                          (int(self.mid[0]+self.scale*self.pointsd[self.polygons[i][2]][0]),int(self.mid[1]+self.scale*self.pointsd[self.polygons[i][2]][1]))],width=0)
    ...
    def direction(self):
        cam = self.camera
        for i in range(len(self.polygons)):
            p1 = self.points[self.polygons[i][0]]
            p2 = self.points[self.polygons[i][1]]
            p3 = self.points[self.polygons[i][2]]
            v12 = [0,0,0]
            v13 = [0,0,0]
            ...
            for p in range(3):
                v12[p] = p2[p]-p1[p]
                v13[p] = p3[p]-p1[p]
            v12 = np.array(v12).reshape(1,3)
            v13 = np.array(v13).reshape(1,3)
            c = np.cross(v12,v13)
            try:
                temp = np.dot(c,cam)/np.abs(c)
                if self.polygons[i][3] == 1:
                    if temp[0][0]+np.abs(p1[0][0])<0:
                        self.liste[i] = True
                    else:
                        self.liste[i] = False
                else:
